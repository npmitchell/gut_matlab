...
%% Add paths from imsane by running setup.m
...
detectOptions = struct( 'channel', channel, 'ssfactor', ssfactor, ...
    'niter', niter, 'niter0', niter0, ...
    'lambda1', lambda1, 'lambda2', lambda2, ...
    'nu', nu, 'smoothing', smoothing, ...
    'post_nu', post_nu, 'post_smoothing', post_smoothing, ...
    ...
    'zdim', 2, ...
    'pre_nu', pre_nu, 'pre_smoothing', pre_smoothing, ...
    'ofn_smoothply', ofn_smoothply, ...
    'mlxprogram', ...
            './surface_rm_resample20k_reconstruct_LS3_1p2pc_ssfactor4.mlx', ...
    'init_ls_fn', 'msls_apical_init.npy', ... % set to none to load prev tp
    'run_full_dataset', false,... % projectDir, ... % set to 'none' for single tp
    'radius_guess', 10, ...
    ...
    'save', true, ... % xyz of the initial guess sphere ;
    'plot_mesh3d', false, ...
    'mask', 'none', ...
...
for t = xp.fileMeta.timePoints
    if ~exist(fullfile(projectDir, [sprintf(fn_output, t) '.h5']), 'file')
        disp(['Did not find file: ', fullfile(projectDir, [sprintf(fn_output, t) '.h5'])])
        xp.loadTime(t);
        xp.rescaleStackToUnitAspect();
        % make a copy of the detectOptions and change the fileName
        detectOpts2 = detectOptions ;
        detectOpts2.fileName = sprintf( fn_output, xp.currentTime ) ;
        ...
        disp(['Found ' fullfile(projectDir, [sprintf(fn_output, t) '.h5'])])
