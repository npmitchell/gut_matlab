...
if do_part2
    ...
    projectDir = '/mnt/crunch/48Ygal4UASCAAXmCherry/201902072000_excellent/Time6views_60sec_1.4um_25x_obis1.5_2/data/deconvolved_16bit/' ;
    meshDir = fullfile(projectDir, 'msls_output_prnun5_prs1_nu0p00_s0p10_pn2_ps4_l1_l1/') ;
    ...
    for ii = 1:length(fns) 
        ...
        name = strsplit(fns(ii).name, '.ply') ;
        name = name{1} ;
        xx = mesh.v(:, 1) / ssfactor ;
        yy = mesh.v(:, 2) / ssfactor ;
        zz = mesh.v(:, 3) / ssfactor ;
        h = trimesh(mesh.f, xx, yy, zz, 'EdgeColor', 'none', 'facecolor', lscolor) ;
        axis equal
        axis off
