...
%% Add paths
dataDir = '/mnt/data/tubular_test/single_coil_dynamic2/' ;
...
for tp = 1:TT
    ...
    if ~exist(meshfn, 'file')  || overwrite
        RR = (1 + sin(((tp-1)/TT-0.25)*2*pi)); % radius of coil (width of lateral motion)
        R1 = 0.25 + 0.15*sin(((tp-1)/TT-0.25)*2*pi); % amount of radial undulation / variation along pathlength
        debug = false ;
        ...
        ww = max(0.02, tp*0.01 + 0.2 + R1 * (cos(0.5*(zz/LL+0.5)*2*pi))+0.1) ;
        plot(zz, ww)
        ylim([0,1])
        pause(2)
...
if ~exist(fullfile(dataDir, 'xp.mat'), 'file') 
    ...
    if ~exist('./masterSettings.mat', 'file') || overwrite 
        ...
        timePoints = 1:20;       % timepoints to include in the analysis
...
tp2do = [t0_for_phi0, tps(tps > t0_for_phi0), fliplr(tps(tps < t0_for_phi0))] ;
for tt = tp2do
...
for tidx = tp2do
    ...
    if ~exist(dataFn, 'file') || ~exist(labelFn, 'file') || overwrite
        ...
        if ~exist(p3dfn, 'file') || overwrite
            ...
            save(p3dfn, 'p3d', 'sizes', 'p2d_zp', 'tri', 'memp2d', 'memp3d', 'p2d_zpTiled', ...
                'bcs', 'memIm')
        else
            load(p3dfn, 'p3d', 'sizes', 'p2d_zp', 'tri', 'memp2d', 'memp3d', 'p2d_zpTiled', ...
                'bcs', 'memIm')
        end
        close all
