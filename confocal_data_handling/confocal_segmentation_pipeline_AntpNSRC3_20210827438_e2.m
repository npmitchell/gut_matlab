...
addpath_recurse(fullfile(codeDir, 'gut_matlab'))
addpath(fullfile(codeDir, 'TexturePatch'))


% AntpNSRC3
dataDir = '/Users/npmitchell/Desktop/AntpNSRC3_48YG4CAAXmCh/' ;
dataDir = [dataDir '20210827438_Antp48YG4CAAXmCh_control_1p2x40x_2um_10mpf_2t5pc_lav4fac2/e2/'] ;
...
% file16name = 'wt_T%03d' ;     
file16name = 'e2_antp_10mpf_T%02d' ;     

% 1.5x40x with 800 pixels
% pix2um = 0.2431478 ; % 202106061730
% pix2um = 0.2427533166 ; % 202106081815
% pix2um = 0.332106383 ; % antpns 202108291112
pix2um = 0.33107221527 ; % antp 20210827438_Antp48YG4CAAXmCh_control_1p2x40x_2um_10mpf_2t5pc_lav4fac2.lif
...
um2pix = 1 / pix2um ;
resolution = [pix2um, pix2um, 2.0] ;
timepoints = 0:26 ;

%% Rename so that first timepoint is 1
% rename_batch_files
...
expMeta.description         = 'mutant Drosophila gut';
...
if useFilteredMesh
    load('./msls_output/medianFilteredMeshes.mat', ...
        'meshFilt', 'faces', 'out_of_frame_idx')
end
for tidx = tidx2do
    tp = timepoints(tidx) ;
    ...
    if ~exist(imfn, 'file') || overwrite
        ...
        if ~useFilteredMesh
            ...
            meshfn = fullfile(mslsDir, sprintf('mesh_%06d.ply', tp)) ;
            mesh = read_ply_mod(meshfn) ;
...
for tp = 2:31
    ...
    axis =3 ;
    normalize = false ;
    flipThroughStackFindLayer(ims1, ['tp=' num2str(tp) ' layer='], ...
        axis, bigstep, fig, normalize)
end
...
for tidx = tidx2do
    ...
    if ~exist(outfn, 'file') || overwrite
        ...
        % try
            ...
            singlePage = imread( ...
                fullfile(textureDir, sprintf('layer0_T%03d_c1.png', tp))) ;
...
% Load an im for size
imfn = fullfile(textureDir, sprintf('slice_T%03d_c%01d.tif', timepoints(1), 1)) ;
im = loadtiff(imfn) ;
...
hold on;
% median
h1med = plot(timestamps, Qct_mk, ':', 'color', colors(1, :)) ;
% Standard deviation and mean
lineProps = {'.-', 'color', colors(1, :)} ;
h1 = shadedErrorBar(timestamps, Qct_ak, Qct_sk, 'lineProps', lineProps) ;
% Standard error
h1e = plot(timestamps, Qct_ak-Qct_ek, '--', 'color', colors(1, :)) ;
plot(timestamps, Qct_ak+Qct_ek, '--', 'color', colors(1, :)) ;
legend([h1med, h1.patch, h1e], {'median$(Q_{xx})$', '$\overline{ Q_{xx} } \pm \sigma_{Q_{xx}}$', ...
    '$\overline{ Q_{xx} }\pm\sigma_{\overline{ Q_{xx}}}$'}, 'interpreter', 'latex')
...
hold on;
% median
h2med = plot(timestamps, Qst_mk,  ':', 'color', colors(2, :)) ;
% Standard deviation and mean
lineProps = {'.-', 'color', colors(2, :)} ;
h2 = shadedErrorBar(timestamps, Qst_ak, Qst_sk, 'lineProps', lineProps) ;
% Standard error
h2e = plot(timestamps, Qst_ak-Qst_ek, '--', 'color', colors(2, :)) ;
plot(timestamps, Qst_ak+Qst_ek, '--', 'color', colors(2, :)) ;
legend([h2med, h2.patch, h2e], {'median$(Q_{yy})$', '$\overline{ Q_{yy} } \pm \sigma_{Q_{yy}}$', ...
    '$\overline{ Q_{yy} }\pm\sigma_{\overline{ Q_{yy}}}$'}, 'interpreter', 'latex')
ylims2 = ylim ;
