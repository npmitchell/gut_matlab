...
% thres A number between 0 and 1 denoting the place in the cdf to cut off
%       the intensity.
...
%             with 1's for peak positions or a label matrix for the
%             centroid method
%
%Example:
%
...
%   adapted from FastPeakFind Ver 1.7 , Date: Oct 10th 2013
%   Extensive modifications by
%   Noah Mitchell (npmitchell@kitp.ucsb.edu)
%
%% defaults
if (nargin < 1)
...
if ndims(d)>2  % in case one uses imread (JPG\PNG\...).
    d=uint16(rgb2gray(d));
end
...
if any(d(:))   %for the case of non zero raw image
    ...
    % Generate threshold using CDF
    [fcdf, xcdf] = ecdf(d(:)) ;
    abovecut = find(fcdf > thres) ;
    thres = xcdf(min(abovecut(1) + 1, length(xcdf) - 1)) ;
...
if (nargin < 1)
    colormap(bone);hold on; plot(cent(1:2:end),cent(2:2:end),'rs');
    hold off; 
end
