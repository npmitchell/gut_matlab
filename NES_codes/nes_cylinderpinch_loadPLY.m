...
for tp = 110
    ...
    try
        disp('Trying to load ply')
        mesh = read_ply_mod(sprintf(rscPlyFn, tp)) ;
    catch
        disp('Could not load ply, building it here')
        % build PLY from matfile
        tmp = load(sprintf(rscMatFn, 110)) ;
        spcm = tmp.spcutMeshSmRSC ;
                
        % check it
        trisurf(spcm.f, spcm.v(:, 1), spcm.v(:, 2), spcm.v(:, 3))
        plywrite_with_normals(sprintf(rscPlyFn, tp), ...
            spcm.f, spcm.v, spcm.vn)
        clearvars tmp
    end
end
