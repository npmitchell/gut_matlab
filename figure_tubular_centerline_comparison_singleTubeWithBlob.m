...
%% Add paths
dataDir = '/mnt/data/tubular_test/tube_centerlines/singleCoilWithBlob/' ;
origpath = '/mnt/data/code/tubular/' ;
cd(origpath)
addpath(origpath) ;
addpath(genpath('utility'))
addpath(fullfile('/mnt/data/code/gut_matlab', 'gptoolbox'))
...
try
    ...
    mesh = read_ply_mod(fullfile(outdir, 'msls_output', 'mesh_ls_000000.ply')) ;
    mesh_sm = read_ply_mod(fullfile(outdir, 'msls_output', 'mesh_000000.ply'))  ;
    load(fullfile(outdir, 'bwskel_centerline.mat'), 'skel','mesh', 'mesh_sm', 'seg')
    load(fullfile(outdir, 'input_centerline.mat'), 'xlin', 'ylin', 'zlin', 'rr') ;
    ...
    R1 = 0.7 ; % amount of radial undulation / variation along pathlength
    debug = false ;
    ...
    rr = [ RR * cos(zz*2*pi/LL); RR * sin(zz*2*pi/LL); zz]' ;

    blobc1 = rr(round(0.6*length(rr)), :) + [0, -0.9, 0] ;
    blobr = .25;
    
    blobc2 = rr(round(0.35*length(rr)), :) + [0, 0.9, 0] ;
    ...
    xpt = blobc1(1) ; ypt = blobc1(2) ; zpt = blobc1(3) ;
    seg = seg + ((segx-xpt).^2 + (segy-ypt).^2 + (segz-zpt).^2 < blobr) ;
    xpt = blobc2(1) ; ypt = blobc2(2) ; zpt = blobc2(3) ;
    seg = seg + ((segx-xpt).^2 + (segy-ypt).^2 + (segz-zpt).^2 < blobr) ;
    seg = seg>0 ;
    ...
    axis off
    set(gcf, 'color', 'white')
    export_fig(gcf, fullfile(outdir, 'mesh_and_skeleton2.png'), '-nocrop', '-r600')

    %% save mesh to disk

    disp('saving mesh and skeleton')
    mkdir(fullfile(outdir, 'mesh_output'))
    plywrite(fullfile(outdir, 'mesh_output', 'mesh_ls_000000.ply'), mesh.f, mesh.v) 
    plywrite_with_normals(fullfile(outdir, 'mesh_output', 'mesh_000000.ply'), mesh_sm.f, mesh_sm.v, mesh_sm.vn) 
    save(fullfile(outdir, 'bwskel_centerline.mat'), 'skel','mesh', 'mesh_sm', 'seg')
    save(fullfile(outdir, 'input_centerline.mat'), 'xlin', 'ylin', 'zlin', 'rr') ;

    % Save segmentation as volume
    writeTiff5D(reshape(uint16(seg*65535), ...
        [size(seg, 1), size(seg, 2), 1, size(seg, 3)]), ...
        'Time_000000_segmentation.tif', 16)
...
mesh_sma = mesh_sm ;
mesh_sma.v = mesh_sm.v + opts.normalShift * mesh_sm.vn ;
...
export_fig(gcf, fullfile(outdir, ...
    sprintf('mesh_and_skeleton_bwskel_%02dstep.png', opts.normalShift)), '-nocrop', '-r600')
...
%% Demonstrate path independence of cutMesh
tubi.setTime(0)
mesh = tubi.getCurrentCutMesh() ;
