function stabilizeImages(fileName, fileNameOut, rgbName, typename, ...
    timePoints, times_todo, t_ref, im_intensity, imref_intensity, ...
    mipDir, mipStabDir, mipsRGBDir, overwrite_mips, overwrite_tiffs)
% STABILIZEIMAGES()
    ...
    if ismember(time, times_todo)
        ...
        if tiff_exists && ~overwrite_tiffs && ~overwrite_mips
            ...
            if ~tiff_exists || overwrite_tiffs
                ...
                disp(['saved image ' sprintf(fileNameOut,time)])
            end
