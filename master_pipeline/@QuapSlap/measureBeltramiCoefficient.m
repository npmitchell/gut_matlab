...
u_material = [refMesh.u(:, 1) * refMesh.ar, refMesh.u(:, 2)] ;
...
for tidx = 1:nTimePoints
    ...
    vUa = reshape(squeeze(vP2d.vU_affine(tidx, :, :)), [], 1) ;
    vV = reshape(vP2d.vV(tidx, :, :), [], 1) ;
    v2d(:, 1) = vUa ;
    v2d(:, 2) = vV ;
    mu_instant(tidx, :) = bc_metric(refMesh.f, v2d, v3d, 3) ;
    mu_material(tidx, :) = bc_metric(refMesh.f, u_material, v3d, 3) ;
    ...
    subplot(1, 2, 1)
    [ax1, ax2, cb1, cb2, mesh1, mesh2] = ...
        twoScalarFieldsOnSurface({refMesh.f, [vU * arelax, vV, 0 * vV]}, ...
        real(mu_instant(tidx, :)), imag(mu_instant(tidx, :)), options) ;
    sgtitle(['$\mu($embedding, Lagrangian pullback$)$, $t = $', ...
        sprintf('%03d', tp-t0), ' ', QS.timeUnits], ...
        'interpreter', 'latex') 
