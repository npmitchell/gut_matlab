function aux_plotCellSegmentation3D(QS, tp, seg3d, imdir, overwrite, xyzlims, vertexBasedTiling)
    ...
    if ~exist(imfn, 'file') || overwrite
        ...
        FF = getframe(gcf) ;
        Fim = FF.cdata ;
        if ~all(size(Fim) == [imWpix,   imHpix, 3])
            Fim = imresize(Fim, [imWpix ,  imHpix]) ;
        end
        imwrite(Fim, imfn) ;
    end
    ...
    for tmp = 1:2
        if ~exist(imfns{tmp}, 'file') || overwrite 
            ...
            imwrite(Fim, imfns{tmp}) ;
        end
    end
    
    %% Nematic anisotropy tensor (like q-weighted Order parameter)  
    imfn = fullfile(imdir, sprintf('cellseg3d_anisotropy_%06d.png', tp)) ;
    clf
    if ~exist(imfn, 'file') || overwrite
        patch('Faces',ff,'Vertices',vv,...
            'FaceVertexCData',QxxV,'FaceColor','flat', ...
            'Edgecolor', 'none');
        cb = colorbar ;
        ylabel(cb, 'cell anisotropy, $Q_{xx}=\left(\sqrt{I_1/I_2}-1 \right)\cos 2\theta$',   'interpreter', 'latex')
        caxis([-1.5, 1.5])
    ...
    imfn = fullfile(imdir, sprintf('cellseg3d_anisotropyBonds_%06d.png', tp)) ;
    clf
    if ~exist(imfn, 'file') || overwrite
        if ~vertexBasedTiling
    ...
    imfn = fullfile(imdir, sprintf('cellseg3d_orderBonds_%06d.png', tp)) ;
    clf
    if ~exist(imfn, 'file') || overwrite
        if ~vertexBasedTiling
