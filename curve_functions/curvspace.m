...
%   modified NPMitchell 2019 for speedup and NaN handling
...
% dist_bet_pts = zeros(len-1, 1) ;
% for k0 = 1:len-1
%     dist_bet_pts(k0) = distance(p(k0,:),p(k0+1,:));
%     % x = p(k0, :) ;
%     % y = p(k0 +1, :) ;
%     % if size(x,2) == 2
%     %    dist_bet_pts(k0) = sqrt((x(1)-y(:,1)).^2+(x(2)-y(:,2)).^2);
%     % elseif size(x,2) == 3
%     %    dist_bet_pts(k0) = sqrt((x(1)-y(:,1)).^2+(x(2)-y(:,2)).^2+(x(3)-y(:,3)).^2);
%     % else
...
% FAST VERSION (NPMITCHELL)
totaldist = nansum(vecnorm(diff(p), 2, 2));
