...
for ii=1:length(fns)
    ...
    if ii == 1  
        ...
        redo_rot_calc = ~exist([rotname '.txt'], 'file') ;
        redo_rot_calc = redo_rot_calc || ~exist([dcomname '.txt'], 'file') ;
        redo_rot_calc = redo_rot_calc || overwrite ;
        if redo_rot_calc
            apfn = fullfile(rootdir, ['Time_' timestr '_c1_stab_Probabilities_apcenterline.h5' ]);
            apdat = h5read(apfn, '/exported_data');
            ...
            if exist([dcomname '.txt'], 'file') && ~overwrite
                dcom = dlmread([dcomname '.txt']) ;
            else
                search4com = true ;
