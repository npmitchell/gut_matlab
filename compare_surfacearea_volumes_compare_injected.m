...
% In this script, we normalize by the mean area, volume, or length at t=0,
% instead of by each individual area, volume, or length. 
% Then we compare the injected embryo's measurements to this ensemble.

clear 
close all

% Select where figure will go
outdir = '/mnt/data/analysis/2020/' ;
...
%% Injection data
injdir = '/mnt/crunch/48Ygal4-UAShistRFP-injected/201804160230_114tp/data/deconvolved_16bit/msls_output/' ;
tf1_inj = 17 ;
tfa_inj = 44 ;
tfp_inj = 49 ;
...
%% Compute mean measurements for ensemble in raw units
v0s = [] ;
a0s = [] ;
l0s = [] ;
...
for mi = 1:length(markers)
    ...
    for j=1:length(these_paths)
        ...
        if exist(writhefn, 'file')
            ...
            l0s = [l0s aplength(t0ind)] ;
        else
            disp('Writhe file NOT found...')
        end
    end
end
...
for build = 1:4
    for plot_injected = 0:1
        ...
        %% Load injection data for comparison
        if plot_injected
            tmp = load([injdir 'surfacearea_volume_stab.mat'], 'vvs', 'aas') ;
            vvs = tmp.vvs ;
            aas = tmp.aas ;

            % offset times
            tt = (1:length(vvs)) - tf1_inj ;

            % Plot results
            axes(ax1)
            plot(tt / 60, vvs(:) / v0, '--', 'color', color1, 'linewidth', lw_mean)
            if build > 1
                plot(tt / 60, aas / a0, '--', 'color', color2, 'linewidth', lw_mean)
            end

            % Plot derivatives
            asmooth = smooth(aas / a0, windowSize) ;
            vsmooth = smooth(vvs / v0, windowSize) ;
            da = gradient(asmooth) ;
            dv = gradient(vsmooth) ;
            axes(ax2)
            vinject = plot(tt(windowSize:end-windowSize) / 60, dv(windowSize:end-windowSize) * 60, ...
                '--', 'color', color1, 'linewidth', lw_mean) ;
            if build > 1
                ainject = plot(tt(windowSize:end-windowSize) / 60, da(windowSize:end-windowSize) * 60, ...
                    '--', 'color', color2, 'linewidth', lw_mean) ;
            end
            ...
            hold on 
            if build == 1
                legend([volumemean vinject], ...
                    {'volume', 'aspirated'}, ...
                    'Location', 'northeastoutside', 'FontSize' , fontsize, 'interpreter', 'latex') ;
            elseif build == 2
                legend([areamean ainject volumemean vinject], ...
                    {'surface area', 'aspirated', 'volume', 'aspirated'}, ...
                    'Location', 'northeastoutside', 'FontSize' , fontsize, 'interpreter', 'latex') ;
            elseif build > 2
                legend([lengthmean linject areamean ainject volumemean vinject], ...
                    {'length', 'aspirated', 'surface area', 'aspirated', 'volume', 'aspirated'}, ...
                    'Location', 'northeastoutside', 'FontSize' , fontsize, 'interpreter', 'latex') ;
            end
        else
            %% LEGEND FOR AXIS 1 -- no aspirated sample label
            axes(ax1)
            hold on 
            if build == 1
                legend([volumemean ], ...
        ...
        if build < 4
            ...
            set(ax1, 'position', axpos_with_writhe)
        end
        ...
        %% Save the figure
        if ~plot_injected
            saveas(gcf, fullfile(outdir, ['area_volume_variation_build' num2str(build) '.pdf']))
            saveas(gcf, fullfile(outdir, ['area_volume_variation_build' num2str(build) '.png']))
        else
            saveas(gcf, fullfile(outdir, ['area_volume_injection_build' num2str(build) '.pdf']))
            saveas(gcf, fullfile(outdir, ['area_volume_injection_build' num2str(build) '.png']))
        end
    end
end
