...
% I tried to add an 8th dataset here (HandGAL4), which has dt = 2 min.
% I think it basically works now with interpolation between timestamps, etc
% But it doesn't really change the result. So we used the version with N=7
% instead.

% SCRIPT FOR MANUSCRIPT FIGURES, VERSION 5
...
tfp_actin = {54, 20};
dt_actin = [1,1] ;
tf1_nuclei = {7, 52, 77-65, 1} ; % artificially offset
tfa_nuclei = {40, 82, 106-65, 8} ; % artificially offset
tfp_nuclei = {40, 90, 104-65, 26} ; % artificially offset
dt_nuclei = [1,1,1,2] ;
...
hist_paths = {[crunch hist_root '201901021550_folded_2part/'...
    ...
    ['/mnt/data/handGAL4klarHandGFPhistGFP/202105072030_1mWGFP/deconvolved_16bit/msls_output/']
    } ;
hist_nUs = {100, 100, 100, 100,} ;
hist_nVs = {100, 100, 100, 100,} ;
hist_shifts = {10, 10, 10, 10,} ;
...
for mi = 1:length(markers)
    ...
    for j=1:length(these_paths)
        ...
        if exist(fn, 'file')
            ...
            times0 = 1:length(areas) ;
            times = dt*(times0 - t0) ;
            times = min(times):max(times) ;
            if dt > 1
                areas = repelem(areas,dt) ;
                volumes = repelem(volumes, dt) ;
                times = [times, max(times)+1:max(times)+dt-1] ;
            end
            ...
            axes(ax1)
            % Plot data
            plot(times / 60, vsmooth, 'Color', [color1 opacity], 'LineStyle', linestyle);
            hold on;
            plot(times / 60, asmooth, 'Color', [color2 opacity], 'LineStyle', linestyle) ;
            ...
            [~, ia] = min(abs(times + t0*dt - ta*dt)) ;
            [~, ip] = min(abs(times + t0*dt - tp*dt)) ;
            ...
            error('where is the surfacearea_volume file?')
            do_update = false; 
        end
        ...
        if exist(writhefn, 'file')
            ...
            if dt > 1
                writhe = repelem(writhe, dt) ;
                aplength = repelem(aplength, dt) ;
            end
            
            assert(length(times) == length(aplength))

            apL = aplength / aplength(t0ind) ;
            dL = gradient(smoothdata(aplength, 'rlowess', 11)) ;
            dLn = gradient(smoothdata(apL, 'rlowess', 11)) ;
            dWr = gradient(smoothdata(writhe, 'rlowess', 11));
...
% Format individual curve figure and save
axes(ax1)
...
ylabel({'rate of change, $\partial_t\tilde{L}$ [hr$^{-1}$]'}, ...
    'Interpreter', 'Latex', 'FontSize', fontsize)
saveas(gcf, fullfile(outdir, ['fig_area_volume_stab_comparison_derivatives_indiv_v5.pdf']))
close all
...
saveas(gcf, fullfile(outdir, ['fig_area_volume_stab_comparison_derivatives_v5.pdf']))
% hold off
%% ADD LEGEND
legend(ax2, [H1 H2 H3 af pf], ...
    ...
    'interpreter', 'latex') ;
outfn = fullfile(outdir, ['fig_area_volume_stab_comparison_derivatives_legend_v5.pdf']) ;
