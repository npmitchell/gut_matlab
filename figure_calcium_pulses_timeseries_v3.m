...
close all
fig = figure('units','centimeters','position',[0,0,16,4]);
subplot(1, 2, 2)
hold on;
if nregions == 3
...
ylabel('fluctuating GCaMP intensity, max$(I)$ [a.u.]', 'interpreter', 'latex')
...
saveas(gcf, fullfile(rootdir, resDirFn, ['posterFigure' exten '.pdf'])) ;
imwrite(rgb, fullfile(rootdir, resDirFn, ['posterImage' exten '.png'])) ;
...
fillAbove0 = upper00 - m00 ;
fillAboveN = upper00 - mn1 ;
fillAboveP = upper00 - mp1 ;
fillBelow0 = lower00 - m00 ;
fillBelowN = lowern1 - mn1 ;
fillBelowP = lowerp1 - mp1 ;
%lineProps = {'-','color', colors(1, :)} ;
%h1=shadedErrorBar(timestamps, m00, [m00+fillAbove0; m00-fillBelow0], 'lineProps', lineProps) ;

%lineProps = {'-','color', colors(2, :)} ;
%h2=shadedErrorBar(timestamps, mn1, [mn1+fillAboveN; mn1-fillBelowN], 'lineProps', lineProps) ;

%lineProps = {'-','color', colors(3, :)} ;
%h3=shadedErrorBar(timestamps, mp1, [mp1+fillAboveP; mp1-fillBelowP], 'lineProps', lineProps) ;
...
ylabel('fluctuating GCaMP intensity, max$(I)$ [a.u.]', 'interpreter', 'latex')
ylim(ylims)
saveas(gcf, fullfile(rootdir, resDirFn, ['posterFigure' exten '_part2.pdf'])) ;
