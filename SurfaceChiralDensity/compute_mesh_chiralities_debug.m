...
path = '/Users/npmitchell/Dropbox/Soft_Matter/UCSB/gut_morphogenesis/data/48Ygal4-UAShistRFP/2016021015120_objFiles/';
% meshes = dir([path, 'cleaned/cleaned_pointCloud*_mesh_ascii.ply']) ;
meshes = dir([path, 'pointCloud*_mesh_ascii.ply']) ;
...
for ii = todo
    try
        ...
        for i = 1:size(face,1)
            ...
            % chi(i) = chiral{i}(1) ;
            c = chiral{i} ;
            chi(i) = 0.5 * (trace(c)^2 - tr(c.*c)^2) ;
        end
        chisums(dmyk) = nansum(chi);
