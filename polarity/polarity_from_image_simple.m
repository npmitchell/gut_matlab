% Simple version of computing local tensor
% edited NPM 2019 for clarity

% Go through all images in stack
...
ilastiksegFolder = [soiDir 'fields_adjusted/data_ilastikseg/'] ;
...
for t=1:size(L, 3)
    ...
    % Save the image 
    saveas(hf, sprintf([outdir 'memaniso_%06d.png'], t))
...
xlim([0, size(L, 3)]) ;
ylim([min(aniso_medians - aniso_unc), 0.1])
