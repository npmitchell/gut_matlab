function wN = findAnnularPathZeroWindingNumber(F, V, startpt, endpt)
...
%       - startpt:   int vertex ID
%       - endpt:     int vertex ID
%
%   OUTPUT PARAMETERS:
%       -wN:    The winding number of the path
%
% NPMitchell 2020, adapted from code by Dillon Cislo 
...
triplot(F, V(:, 1), V(:, 2), '.')
plot(V(startpt, 1), V(startpt, 2), 'o')
plot(V(startpt, 1), V(startpt, 2), 's')
error('here')

% Verify the path origin and termination are on boundary ------------------
flip_path = false ;
if ~ismember(startpt, innerIDx)
...
if ~ismember(endpt, outerIDx)
    error('Path must terminate on the outer boundary');
end

% Find the shortest path as discrete geodesic -----------------------------
...
%--------------------------------------------------------------------------
% Calculate the Winding Number to check the answer
%--------------------------------------------------------------------------
