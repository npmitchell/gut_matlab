...
for refID = [2,1]
    ...
    for clipyPairIdx = [2]
        ...
        for avgID = 1:length(avgMin)
            ...
            noise_model = struct('background_noise', bgVals, ...
                'brightest_DVaveraged_signal', maxVals, ...
                'linear_fit_for_signal_to_noise', pp, ...
                'readme', 'fit for signal as a function of noise used to assign an absolute scale to the calcium activity based on WT (Antp heterozygous) behavior') ;
            ...
            subplot(3, 1, 3)
            cla
            avgactC = mean(statAllControl, 2, 'omitnan') ;
            stdactC = std(statAllControl, 0, 2, 'omitnan') ;
            lineProps = {'-','color', colors(1, :)} ;
            finalNorm = max(avgactC(:)) ;
            factorC = 1.0 / finalNorm ;
            avgactC = movmean(avgactC, 3) ;
            stdactC = movmedian(stdactC, 15) ;
            h1=shadedErrorBar(xfixedGlobal, avgactC*factorC, stdactC*factorC, 'lineProps', lineProps) ;
            ...
            %% Save the kymo for controls
            normalization_factor_control = factorC ;
            normalization_factor_mutant = factor ;
            outfn = fullfile(outdir, ['gcamp_mean_results' sprintf(['_clipY%d_avg%d' refStr '.mat'], clipyPairIdx, avgMin(avgID))]) ;
            readme = ['xfixedGobal parameterizes the position along the AP axis of the '...
                'midgut (projected position, in the Eulerian frame), and '...
                'fixTimeStamps parameterizes time since the onset of the anterior constriction.'...
                ' Then, kymoMControl is the corresponding GCaMP activity (defined below) in this spacetime grid for Antp heterozygotes. '...
                'kymoM is the corresponding GCaMP activity in the spacetime grid for homozygotes (mutant for Antp). ' ...
                'In order to remove background signal, the activity is defined as '...
                'deltaI = |I1-I2| + |I1-I3| + |I2-I3|, which captures the transient activity between rapidly acquired stacks I1, I2, and I3 '...
                'So, each timepoint measurement is a summed difference between frames taken 9-10 seconds apart. '...
                'In order to compare absolute scales in intensity, we calibrated the expected maximum intensity based on the signal-to-noise ratios observed in the heterozygotes. '...
                'This information is provided in noise_model. nsamplesControl is a spacetime grid of how many embryos participate in the mean measurement kymograph kymoMControl. '...
                'Similarly, nsamples is a spacetime grid of how many samples participate in the mean measurement reported in kymoM.' ];
            save(outfn, 'kymoMControl', 'nsamplesControl', 'kymoM', 'nsamples', ...
                'xfixedGlobal', 'fixTimeStamps', ...
                'normalization_factor_mutant', ...
                'normalization_factor_control', ...
                'noise_model', 'readme') ;
