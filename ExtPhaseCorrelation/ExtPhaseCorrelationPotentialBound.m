function [deltaX , deltaY, dxint, dyint] = ExtPhaseCorrelationPotentialBound(img1, img2, potential, dim, varargin)
%Description:
...
%
%TODO:
%   - Check the rationale for subtracting 1 when outside image...
...
% -------
% deltaX : float
%   subpixel resolution translation between img1 and img2
% deltaY : float
%   subpixel resolution translation between img1 and img2
% x : int
...
if check_bounds
    kii = 2 ;
    force_exit = false ;
    while deltaX < lowerx || deltaX > upperx || deltaY < lowery || deltaY > uppery || force_exit
        if verbose
        ...
        %if the peaks outsize the image, then use xs-1 and/or ys-1 for
        % the two side-peaks
        if xsp > M
            xsp = M-1;  % shouldn't this be xsp - M = 1?
        end
        if ysp > N
            ysp = N-1;  % shouldn't this be ysp - N = 1?
        end
