...
tmp = imread([mips,sprintf(name_1,times(1))]) ;
im_1 = zeros([size(tmp) length(times)]) ;
% preallocate im_2 for speed
tmp = imread([mips,sprintf(name_in,times(1))]) ;
im_2 = zeros([size(tmp) length(times)]) ;
for time_id = 1 : length(times)
    time = times(time_id);
    im_1(:,:,time_id) = imread([mips,sprintf(name_1,time)])+imread([mips,sprintf(name_11,time)]);
    im_2(:,:,time_id) = imread([mips,sprintf(name_in,time)])+imread([mips,sprintf(name_21,time)]);
end
...
for time = 1 :NTimes+0
    [shiftx,shifty,~] = xcorr2fft(im_1(:,:,time), im_1(:,:,t_ref+1));
    shifts(time).x_1 = shiftx;
    shifts(time).y_1 = shifty; 
    
    [shiftx,shifty,~] = xcorr2fft(im_2(:,:,time), im_2(:,:,t_ref+1));
...
stackSize = 0 ;
name_ref = sprintf(dummyName,times(t_ref));
while ~done
